    ...
    const fetchEvento = useCallback(async () => {
        ...
        try {
            ...
            if (!isNaN(eventoId)) {
                ...
                setData(
                    `${String(dateFromApi.getUTCDate()).padStart(2, "0")}/${String(
                        dateFromApi.getUTCMonth() + 1
                    ).padStart(2, "0")}/${dateFromApi.getUTCFullYear()}`
                );
                setHorario(fetchedData.horario.substring(0, 5));
                setClienteId(fetchedData.cliente_id);
                setClienteNome(
                    fetchedData.nome_cliente || "Nenhum cliente selecionado"
                );
            }
        } catch (error: any) {
    ...
    if (loading && !evento) {
        return (
            <ActivityIndicator
                size="large"
                color="#fff"
                style={{ flex: 1, backgroundColor: PALETTE.AzulEscuro }}
            />
        );
    }
                    ...
                    <View style={styles.formContainer}>
                        ...
                        <View style={styles.inputGroup}>
                            <TouchableOpacity
                                ...
                                onPress={() => isEditing && setModalClientesVisible(true)}
                                disabled={!isEditing}
                            >
                                <Feather
                                    name="user"
                                    size={20}
                                    color={isEditing ? PALETTE.CinzaClaro : PALETTE.Branco}
                                    style={styles.icon}
                                />
